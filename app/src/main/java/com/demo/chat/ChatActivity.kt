...
class ChatActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tvcontactName: TextView = findViewById(R.id.tvName)
        tvcontactName.text = contactName
        val contactImage = intent.getIntExtra("Contact IMAGE", 0)
        val contactImageView: ImageView = findViewById(R.id.contactActivityIv)
        contactImageView.setImageResource(contactImage)

        val user = FirebaseAuth.getInstance().currentUser
        val userId = user?.uid

        chatRecyclerView = findViewById(R.id.rvMessage)
